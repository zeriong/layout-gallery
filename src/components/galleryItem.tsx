...
export const GalleryItems = (props: { i: number, val: { img: string, name: string, desc: string, path: string } }) => {
    ...
    return (
        <li
            ...
            onClick={onClick}
        >
            <section className={`transition-all duration-300 w-full h-full bg-white rounded-lg overflow-hidden justify-center text-white ${onMouse ? "bg-white text-black" : "bg-primary-default"}`}>
                <figure>
                    <img src={props.val.img} alt={""} className={`h-4/6 w-full`}/>
                </figure>
                <article
                    className="p-2 text-center sm:p-4 h-2/6 overflow-hidden relative flex flex-col sm:text-start sm:items-start justify-center sm:justify-start items-center">
